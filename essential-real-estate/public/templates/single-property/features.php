...
<div class="property-info-tabs property-tab mg-bottom-45 sm-mg-bottom-25">
    ...
    <ul id="ere-features-tabs" class="nav nav-tabs">
        ...
        <?php if ($property_features): ?>
            <li><a data-toggle="tab" href="#ere-features"><?php esc_html_e('Features', 'essential-real-estate'); ?></a></li>
        <?php endif; ?>
        <?php if (!empty($property_video)) : ?>
            <li><a data-toggle="tab" href="#ere-video"><?php esc_html_e('Video', 'essential-real-estate'); ?></a></li>
        <?php endif; ?>
        <?php
        if ((!empty($property_image_360) || !empty($property_virtual_tour)) && ($property_virtual_tour_type == '0' || $property_virtual_tour_type == '1')): ?>
            <li><a data-toggle="tab"
                   href="#ere-virtual_tour_360"><?php esc_html_e('Virtual Tour', 'essential-real-estate'); ?></a></li>
        <?php endif; ?>
    </ul>
            ...
            <ul class="list-2-col ere-property-list">
                ...
                <?php if (!empty($property_land)): ?>
                    <li>
                       ...
                       <span><?php $measurement_units = ere_get_measurement_units();
                           echo sprintf("%s %s", ere_get_format_number($property_land), $measurement_units); ?></span>
                    </li>
                <?php endif; ?>
                ...
                if(count($additional_fields)>0):
                    foreach ($additional_fields as $key => $field):
                        $property_field= get_post_meta(get_the_ID(), $field['id'], true);
                    ...
                    if (is_taxonomy_hierarchical('property-feature') && count($child_items)>0) {
                        foreach ($parents_items as $parents_item) {
                            ...
                            foreach ($child_items as $child_item) {
                                if ($child_item->parent == $parents_item->term_id) {
                                    ...
                                    echo  '<div class="col-md-3 col-xs-6 col-mb-12 mg-bottom-10">';
                                    if ( in_array( $child_item->term_id, $features_terms_id ) ) {
                                        echo  '<a href="'. esc_url($term_link) .'" class="feature-checked"><i class="fa fa-check-square-o"></i> ' . $child_item->name . '</a>';
                                    }
                                    else
                                    {
                                        echo  '<a href="'. esc_url($term_link) .'" class="feature-unchecked"><i class="fa fa-square-o"></i> ' . $child_item->name . '</a>';
                                    }
                                    echo '</div>';
                                };
                            };
                        ...
                        foreach ($parents_items as $parents_item) {
                            ...
                            if ( in_array( $parents_item->term_id, $features_terms_id ) ) {
                                echo '<a href="'. esc_url($term_link) .'" class="feature-checked"><i class="fa fa-check-square-o"></i> ' . $parents_item->name . '</a>';
                            }
                            else
                            {
                                echo '<a href="'. esc_url($term_link) .'" class="feature-unchecked"><i class="fa fa-square-o"></i> ' . $parents_item->name . '</a>';
                            }
                            echo '</div>';
                        };
                        echo '</div>';
